    ...
    return Promise.all(promises).then(function (items) {
        ...
        items.forEach(function (item) {
            ...
            htmlContent = htmlContent.replace(regex, function (tag, pre, url, post) {
                ...
                var result = '<div class="img-container" style="width:'+item.width+'px;">' +
                                '<img' +="" pre="" 'data-src="' + url + '" '="" post="" height="' + item.height + '" width="' + item.width + '" style="padding-top:' + (item.height / item.width * 100) + '%;background:' + item.gradient + '">' +
                             '</img'></div>';

                if (config.noscript) {
                    result = '<noscript><img' +="" pre="" 'src="' + url + '" '="" post="" height="' + item.height + '" width="' + item.width + '" noscript=""></img'></noscript>' + result;
                }
